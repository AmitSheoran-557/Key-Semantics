...
const KeyFaq = () => {
    ...
    return (
        <div className='flex flex-col justify-center items-center mx-auto w-full mt-[164px]'>
            <div className='w-full mx-auto max-w-[1220px] flex flex-col text-center items-center justify-center relative'>
                <div className="flex flex-wrap w-full">
                    <div className='lg:w-6/12 px-3 w-full'>
                        <img className='max-w-[600px] lg:min-w-[600px] max-h-[337px] xl:min-h-[337px] w-full' src="./assets/image/png/faq-img.png" alt="faq-img" />
                    </div>
                    <div className='lg:w-6/12 ps-12 w-full'>
                        <CustomHeading tittle="Key Q&A" className="!text-start !mb-2" />
                        {FAQ_LIST.map((obj, index) => (
                            <div key={index} className='lg:mb-4 mb-3'>
                                <ul>
                                    <li className='text-blue text-start list-disc lg:text-base text-sm !leading-[175%]'>
                                        {obj.para}
                                    </li>
                                </ul>
                            </div>
                        ...
                        <div className="w-[552px]">
                            {ACCORDION_DATA_LIST.map((item, index) => (
                                <div key={index}>
                                    <div className="flex items-center justify-between bg-gray-100 p-3 cursor-pointer" style={{ height: '60px', transition:"all 300ms linear",}} onClick={() => handleToggle(index)}>
                                        <h2 className="text-lg font-medium">{item.title}</h2>
                                        <div className={`transform transition-transform ${openIndex === index ? 'rotate-180' : ''}  ease-linear duration-300`}>
                                    ...
                                    {openIndex === index && (
                                        <div className="bg-gray-50 pt-1">
                                            <p className="lg:text-base text-sm text-start">{item.contentAbove}</p>
                                            <p className="lg:text-base text-sm text-start lg:mb-4 mb-3">{item.contentAboveTwo}</p>
                                            <div className="flex gap-4">
                                                <img className='max-w-[268px] w-full max-h-[150px]' src={item.image} alt="accordion-img-1" />
                                                <img className='max-w-[268px] w-full max-h-[150px]' src={item.imageTwo} alt="accordion-img-2" />
                                            </div>
                                            <p className="lg:text-base text-sm text-start mt-4">{item.contentBelow}</p>
                                        </div>
                                    )}
                                </div>
                            ))}
